...
if(isset($_POST['submit']) && CSRF::validateToken($_POST['token'])) {
    ...
    if (isset($_FILES['files']) && $_FILES['files']['error'][0] !== 4) { // 4: UPLOAD_ERR_NO_FILE
...
<div class="container">
    <div class="page-title">
        ...
        <a href="/admin/products/imports" class="btn btn-sm btn-outline-primary float-end"><i class="fas fa-plus"></i> Import</a>
        </h3>
    </div>
    <?php if($edit): ?>
        <div class="card">
...
if (!empty($items[0]['images'])) {
    $unserialized = @unserialize($items[0]['images']);
        ...
        <div class="row">
            <?php foreach ($images as $img): ?>
                <div class="col-md-3 mb-2">
                    <img src="/<?= htmlspecialchars($img) ?>" class="img-thumbnail" style="max-height: 150px;">
                </div>
            <?php endforeach; ?>
        </div>
    </div>
                    ...
                    <tbody>
                        <?php if(isset($items)): ?>
                            <?php foreach($items as $item): ?>
                                <tr>
                                    <td><?= $item['title'] ?></td>
                                    <td>VND <?= number_format($item['price'], 2) ?></td>
